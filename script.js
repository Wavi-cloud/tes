 ...
 *        Banner swipe + klik, Admin simpel, KTM discount, dll.
 */
...
function init() {
    ...
    // Mulai flash sale rotasi (setiap 3 jam)
...
// ============================================================
// FLASH SALE ROTASI (otomatis berganti setiap 3 jam)
// ============================================================
function startFlashSaleRotation() {
    // Jalankan pertama kali
    rotateFlashSale();
    // Set interval 3 jam (10800000 ms) — untuk demo kita pakai 30 detik agar terlihat
    // Ganti ke 10800000 untuk production
    flashSaleInterval = setInterval(rotateFlashSale, 30000); // 30 detik untuk demo
}

function rotateFlashSale() {
    // Pilih 5 produk random untuk flash sale
    // Reset semua produk yang sedang flash
    products.forEach(p => {
    ...
    // Ambil 5 produk random (kecuali yang sudah student discount? boleh campur)
    const available = products.filter(p => p.discountType !== 'student'); // hindari konflik
    const shuffled = available.sort(() => 0.5 - Math.random());
    const selected = shuffled.slice(0, 5);

    // Beri diskon flash sale (20-40% random)
    selected.forEach(p => {
        p.discountType = 'flash';
        p.discount = Math.floor(Math.random() * 21) + 20; // 20-40%
    });
...
function attachEvents() {
    ...
    if (slides.length > 0) {
        ...
        track.addEventListener('click', () => clearInterval(interval));
    }
